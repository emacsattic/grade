...
(defcustom grade-include-blank-scores nil
  "Non-nil means insert blank scores in the reports."
  :group 'grade
  :type 'boolean)
...
(defvar grade-hidden-columns nil)
(defvar grade-hidden-rows nil)
...
(defun grade-score-list ()
  "Return the possible scores,
in the form ((score-name . maximum-score-value) ...)"
  (let ((info nil)
        (i))
    (if grade-use-studentid (setq i 4) (setq i 3))
    (save-excursion
      (grade-goto-column-nohide i)
      (while (<= i grade-total-columns)
        (let ((score (string-to-number (grade-column-points))))
          (if (> score 0)
              (grade-add-to-list 
               'info 
               (cons (grade-column-name) score)))
          (grade-forward-column-nohide 1)
          (setq i (1+ i))))
      info)))
...
(defun grade-display-formula ()
  "Display the formula for the current column"
  (interactive)
  (let ((name (grade-column-name)))
    (save-excursion
      (if (re-search-forward (concat "^" name "=") (point-max) t)
...
(defun grade-find-next-invisible ()
  (save-excursion
    (while (and (not (eobp))
...
(defun grade-alphabetize (arg)
  ...
  (let ((pt (point))
    ...
    (grade-show-all-save-hidden-info)
    (if arg
  ...
  (let ((rownum (grade-row-number))
    ...
    (when (or (not (interactive-p)) (grade-in-table-p))
      (if (or (< rownum 3) (= rownum grade-total-rows))
        ...
        (setq grade-hidden-rows (cons (grade-student-name) grade-hidden-rows))
        (save-excursion
          (beginning-of-line)
          (when (looking-at "\\*")
              (delete-char 1)
...
(defun grade-hide-column ()
  ...
  (let ((r)
    ...
    (if (or (< c 3) (and grade-use-studentid (= c 3)))
        (message "This column cannot be hidden")
      (setq grade-hidden-columns (cons (grade-column-name) grade-hidden-columns))
      (save-excursion
        (grade-goto-first-row)
        (grade-goto-cell-beginning)
        (grade-next-line -1)
...
(defun grade-rehide-hidden-columns ()
  (let ((cols (copy-sequence grade-hidden-columns))
        (col 0))
    (setq grade-hidden-columns nil)
    (save-excursion
      (grade-goto-table)
      (while (< col grade-total-columns)
        (if (member (grade-column-name) cols)
            (grade-hide-column)
          (grade-forward-column))
        (setq col (1+ col))))))

(defun grade-rehide-hidden-rows ()
  (let ((rows (copy-sequence grade-hidden-rows))
        (row 0))
    (setq grade-hidden-rows nil)
    (save-excursion
      (grade-goto-table)
      (while (< row grade-total-rows)
        (if (member (grade-student-name) rows)
            (grade-hide-student)
          (grade-forward-row))
        (setq row (1+ row))))))

(defun grade-rehide-all ()
  (grade-rehide-hidden-columns)
  (grade-rehide-hidden-rows))
...
(defun grade-show-all-save-hidden-info ()
  "Show all hidden information."
  (let ((ol (overlays-in (point-min) (point-max))))
    (while ol
      (delete-overlay (car ol))
...
(defun grade-show-all-students ()
  (interactive)
  (grade-show-all-save-hidden-info)
  (grade-rehide-hidden-columns)
  (setq grade-hidden-rows nil))

(defun grade-show-all-columns ()
  (interactive)
  (grade-show-all-save-hidden-info)
  (grade-rehide-hidden-rows)
  (setq grade-hidden-columns nil))
    ...
    (if (string= str "AVG(*)")
      ...
      (with-temp-buffer
        ...
        (while (search-forward "<" nil t)
          (let ((pt (point)))
            (search-forward ">")
            (setq comps (cons (buffer-substring pt (1- (point))) comps))))))
    comps))

(defun grade-eval-avg-cell (comps scores &optional maxscores)
  (let ((total 0)
        (score 0))
    (unless maxscores
      (setq maxscores (grade-score-list)))
    (if (eq comps t)
        (setq comps (mapcar 'car maxscores)))
    (while comps
      (let* ((scorename (car comps))
             (newscore (cdr (assoc scorename scores))))
        (unless (string= newscore "")
          (setq total (+ total (cdr (assoc scorename maxscores))))
          (setq score (+ score (string-to-number newscore))))
        (setq comps (cdr comps))))
    (if (= total 0)
        (grade-insert-score "")
      (grade-insert-score (number-to-string (* 100 (/ (* 1.0 score) total)))))))

(defun grade-eval-column ()
  "Evaluate the entire column."
  (let* ((formula (grade-column-formula))
         (comps (if (string-match "^AVG" formula)
                    (grade-find-avg-comps formula)
                  nil))
         (maxscores (if comps (grade-score-list) nil))
         (gresult)
         (gvalues)
         (i 3))
    (grade-goto-row 3)
    (while (< i (- grade-total-rows grade-invisible-students))
      (if comps
          (grade-eval-avg-cell comps (grade-student-scores) maxscores)
...
(defun grade-tex-insert-studentinfo (info beg end gscw)
  (let ((it))
    ...
    (while (<= beg end)
      ...
      (unless (and
               (not grade-include-blank-scores)
               (string= (nth 2 it) ""))
...
(if grade-mode-map
  ...
  (define-key grade-mode-map "f" 'grade-display-formula)
  ;; Moving information
  ...
  (define-key grade-mode-map "\C-c|"  'grade-show-all-columns)
  (define-key grade-mode-map "\C-c_"  'grade-show-all-students)
...
(easy-menu-define grade-mode-menu grade-mode-map "Grade mode menu"
  '("Grade"
    ...
    ("Hiding"
     ...
     ["Show all hidden columns" grade-show-all-columns t]
     ["Show all hidden students" grade-show-all-students t]
...
"
  ...
  (make-variable-buffer-local 'grade-include-blank-scores)
  (make-variable-buffer-local 'grade-hidden-rows)
  (make-variable-buffer-local 'grade-hidden-columns)
